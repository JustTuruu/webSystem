...
//   },
// };
// src/team6/utils/api.js

const API_BASE_URL = "https://todu.mn/bs/lms/api/v1"; // Adjust if Swagger shows different path

// Token хадгалах (жишээ нь багшийн эрх)
...
export async function getVariants(examId) {
  const res = await fetch(`${API_BASE_URL}/exams/${examId}/variants`, {
    headers: headers(),
  });
  if (!res.ok) throw new Error("Вариантуудын мэдээлэл авч чадсангүй");
  return res.json();
}

// ✅ Нэг вариант авах
export async function getVariant(examId, variantId) {
  const res = await fetch(
    `${API_BASE_URL}/exams/${examId}/variants/${variantId}`,
    { headers: headers() }
  );
  if (!res.ok) throw new Error("Вариантын мэдээлэл олдсонгүй");
  return res.json();
}

// ✅ Вариант нэмэх
export async function createVariant(examId, payload) {
  const res = await fetch(`${API_BASE_URL}/exams/${examId}/variants`, {
    method: "POST",
    headers: headers(),
    body: JSON.stringify(payload),
  });
  if (!res.ok) throw new Error("Вариант нэмэхэд алдаа гарлаа");
  return res.json();
}

// ✅ Вариант засах
export async function updateVariant(examId, variantId, payload) {
  const res = await fetch(
    `${API_BASE_URL}/exams/${examId}/variants/${variantId}`,
    {
      method: "PUT",
  ...
  if (!res.ok) throw new Error("Вариант шинэчлэхэд алдаа гарлаа");
  return res.json();
}
