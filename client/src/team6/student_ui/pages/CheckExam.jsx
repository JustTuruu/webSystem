...
const CheckExam = () => {
  ...
  useEffect(() => {
    ...
    const storedAnswers = JSON.parse(sessionStorage.getItem(`exam_${exam_id}_answers`) || '{}');
    
    const loadedQuestions = storedQuestionIds.map(id => mockQuestionBank.find(q => q.id === id)).filter(Boolean);
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Мэдээлэл олдсонгүй</h2>
  ...
  const checkAnswer = (question, studentAnswer) => {
    ...
    if (question.type === 'text_answer') {
      const userLower = studentAnswer.toLowerCase().trim();
      return question.acceptableAnswers?.some(
        acceptable => acceptable.toLowerCase().trim().includes(userLower) ||
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Зөв хариултууд</h1>
            ...
            return (
              ...
              >
                ...
                <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                  <div className="text-sm text-gray-600 mb-1">Таны хариулт:</div>
                  <div className={`font-medium ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>
                    {question.type === 'multiple_correct' && Array.isArray(studentAnswer) ? (
                      ...
                      studentAnswer || <span className="text-gray-400">Хариулаагүй</span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                    ...
                    {question.acceptableAnswers && question.acceptableAnswers.length > 0 && (
                      <div className="mt-2 text-sm text-green-600">
                        Бусад зөв хариултууд: {question.acceptableAnswers.join(', ')}
                      </div>
                    )}
                  </div>
                )}
                ...
                {(question.type === 'multiple_choice' || question.type === 'multiple_correct') && (
                  <div className="mt-4 space-y-2">
                    {question.options.map((option, idx) => {
                      const isStudentChoice = question.type === 'multiple_correct' 
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            {isStudentChoice && !isCorrectOption && <span className="text-red-600">✗</span>}
                            <span className={`font-medium ${
                              isCorrectOption ? 'text-green-800' : isStudentChoice ? 'text-red-800' : 'text-gray-700'
                            }`}>
                              {option}
                            </span>
                          </div>
